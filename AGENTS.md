...
- Prefer `package.json`, `deno.json`, `wrangler.jsonc`, and `src/content.config.ts` over prose when
  they differ.
...
- Use `pnpm run lint` for formatting, linting, and type checking, but expect it to modify files
  because it runs `deno lint --fix`.
...
- `keystatic.config.ts` is the content editor config; post content is stored in
  `src/content/posts/*`.
...
- Build and preview both depend on `wrangler types && astro sync`; do not skip `sync` when worker
  types are stale.
- The site targets Cloudflare Pages/Workers; `wrangler.jsonc` points production at
  `dist/_worker.js/index.js` and disables `workers_dev` and `preview_urls`.
- Deno formatting is configured for 2-space indent, no semicolons, double quotes, and
  `lineWidth: 100`.
- `deno lint` has repo-specific rule settings in `deno.json`; keep new code compatible with those
  rules.
